...
describe('RunGovCyPdfParsingUseCase', () => {
  ...
  let govCyPdfHotelsService: {
    ...
    >;
    parsePdfFiles: jest.Mock<
      Promise<IRecognizedGovCyHotelRecord[]>,
      [
        IDownloadedGovCyPdfFile[],
        (
          | ((parsedHotels: IRecognizedGovCyHotelRecord[]) => Promise<void>)
          | undefined
        )?,
      ]
    >;
  ...
  it('parses an uncached file and saves its parsing summary in parsed_files', async () => {
    ...
    govCyPdfHotelsService.parsePdfFiles.mockImplementation(
      async (_downloadedPdfFiles, onParsedBatch) => {
    ...
    expect(
      parsedFilesService.readManyByFileNamesAndParsedAtFrom,
    ...
    expect(govCyPdfHotelsService.parsePdfFiles).toHaveBeenCalledWith(
      [downloadedPdfFileFixture],
      expect.any(Function),
    );
    expect(
  ...
  it('continues from the first uncached file when previous files are already cached', async () => {
    const secondDiscoveredPdfFileFixture: IDiscoveredGovCyPdfFile = {
      ...
      pdfUrl:
        'https://www.gov.cy/app/uploads/sites/26/2026/04/HOTELS_PARALIMNI_8.4.2026.pdf',
      region: 'PARALIMNI',
    };
    const secondDownloadedPdfFileFixture: IDownloadedGovCyPdfFile = {
      ...secondDiscoveredPdfFileFixture,
      localPath:
        '/opt/hr-core/data/files/2026-04-08/HOTELS_PARALIMNI_8.4.2026.pdf',
    };
    ...
    govCyPdfHotelsService.downloadPdfFiles.mockResolvedValue([
      secondDownloadedPdfFileFixture,
    ]);
    govCyPdfHotelsService.parsePdfFiles.mockImplementation(
      async (_downloadedPdfFiles, onParsedBatch) => {
        await onParsedBatch?.([secondRecognizedHotelFixture]);
  ...
  it('reuses a single in-flight parsing operation for concurrent calls', async () => {
    let resolveParsingPromise:
      | ((value: IRecognizedGovCyHotelRecord[]) => void)
      | null = null;
    const parsingPromise = new Promise<IRecognizedGovCyHotelRecord[]>(
      (resolve) => {
